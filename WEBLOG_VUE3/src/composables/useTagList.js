...
export function useTagList() {
    ...
    const removeTab = (path,isUpdate) => {
        ...
        // 检查标签是否存在
        const tabIndex = tabList.value.findIndex(tab => tab.path === path);
        if (tabIndex === -1) {
            console.log('Tab not found, cannot remove:', path);
            return;
        }

        let tabs = tabList.value
        console.log('removeTab---tabs ', tabs)
        //当前被选中的tab标签
        ...
        if (actTab == path) {
            ...
            tabs.forEach((tab, index) => {
                ...
                if (tab.path == path) {
                    ...
                    console.log('removeTab---actTabNew1 ', nextTab)
                    if (nextTab) {
                        actTab = nextTab.path
                        console.log('removeTab---actTabNew2 ', nextTab)
                    }
                }
            })
        }

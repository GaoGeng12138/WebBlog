  ...
  <div class="admin-list-page admin-comment-list-page min-h-screen bg-transparent p-6">
    ...
    <div class="admin-card overflow-hidden">
      <el-table
        ...
        v-loading="loading"
        style="width: 100%"
        class="comment-table hidden md:block"
      ...
      <div class="md:hidden px-4 pb-3 pt-1">
        <div class="flex flex-col gap-3 rounded-2xl border border-[rgba(149,171,210,0.16)] bg-gradient-to-r from-[#f7fbff] to-[#eef4ff] p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
          <div class="flex items-center justify-between">
            <div class="text-sm font-semibold text-slate-700">共 <span class="text-lg text-blue-600">{{ total }}</span> 条评论</div>
          </div>
        </div>
      </div>
      ...
      <div class="md:hidden px-4 pb-4 space-y-3">
        <article
          v-for="row in commentList"
          :key="row.id"
          class="admin-mobile-card admin-mobile-card--comment rounded-2xl border border-[rgba(149,171,210,0.16)] bg-white p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]"
        >
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                <strong class="text-base text-slate-900">{{ row.articleTitle || '未知文章' }}</strong>
                <el-tag :type="getStatusMeta(row.status).type" effect="light" round size="small">
                  {{ getStatusMeta(row.status).text }}
                </el-tag>
              </div>
              <p class="mt-1 text-xs text-slate-500">文章 ID：{{ row.articleId }} · {{ row.nickname || '匿名用户' }}</p>
              <p class="mt-2 text-sm leading-6 text-slate-600">{{ row.content || '暂无内容' }}</p>
              <div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                <span class="rounded-full bg-slate-100 px-2.5 py-1">{{ formatDateTime(row.createTime) }}</span>
                <span class="rounded-full bg-slate-100 px-2.5 py-1">点赞 {{ row.likeCount ?? 0 }}</span>
              </div>
            </div>
          </div>

          <div class="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-2">
            <span class="text-sm font-medium text-slate-700">置顶</span>
            <el-switch
              :model-value="!!row.isTop"
              inline-prompt
              active-text="是"

        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div v-else>
            <div v-if="articles.length === 0" class="py-16 text-center">
              ...
              <p class="mt-1 text-sm text-gray-500">目前还没有发布任何文章。</p>
            </div>
...
async function load() {
  ...
  const userId = ''
  try {
    const res = await getArticlePageList({ current: page.value, size: size.value, name: keyword.value,userId })
    if (res && res.success) {
...
// Simplified grid layout - always 2 columns on medium screens and up
const gridStyle = computed(() => ({
  gridTemplateColumns: `repeat(${Math.min(2, articles.value.length)}, minmax(0, 1fr))`
}))
</script>
...
- Docker 部署后端时，默认走 `prod` profile
- 生产配置文件为：
  - `weblog-web/src/main/resources/application-prod.yml`
- 当前默认配置：
  - MySQL：`49.232.228.55:3306`
  - MinIO：`49.232.228.55:9000`
  - MinIO AccessKey：`admin`
  - MinIO SecretKey：`95833414`

### 关于 127.0.0.1

- 如果后端也运行在 Docker 容器中，`127.0.0.1` 指向的是后端容器自己，不是宿主机上的 MySQL / MinIO
- 所以按你现在这种部署方式：
  - MySQL、MinIO 用 `docker run` 单独起
  - 后端也单独容器化
  - 此时不建议用 `127.0.0.1`
- 目前建议直接使用服务器地址：`49.232.228.55`
